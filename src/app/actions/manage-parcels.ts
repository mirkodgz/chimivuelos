...
export async function deleteParcel(id: string) {
    ...
    try {
        ...
        const { error } = await adminSupabase.from('parcels').delete().eq('id', id)
        if (error) throw error
        await recordAuditLog({
            actorId: user.id,
            action: 'delete',
...
export async function deleteParcelDocument(id: string, docPath: string) {
    const supabase = supabaseAdmin
    const { data: existing } = await supabase.from('parcels').select('documents').eq('id', id).single()
    if (!existing) return { error: 'Parcel not found' }
    const newDocs = existing.documents.filter((d: {path: string}) => d.path !== docPath)
    const { error } = await supabase.from('parcels').update({ documents: newDocs }).eq('id', id)
...
export async function updateParcelStatus(id: string, status: string) {
    ...
    try {
        ...
        const { error } = await adminSupabase.from('parcels').update({ status }).eq('id', id)
        if (error) throw error
        await recordAuditLog({
            actorId: user.id,
            action: 'update',
    ...
    try {
        ...
        if (parcel.agent_id) {
            const { data: agentData } = await supabase
                ...
                .single()
            if (agentData) {
                (parcel as Parcel).agent = agentData
            }
        }
        ...
        ; (parcel as Parcel).linked_expenses = (linkedExpenses || []) as CorporateExpense[]

        return { success: true, parcel: parcel as Parcel }
    } catch (error) {
        console.error('Error in getParcelFullDetails:', error)
...
export async function getParcelByCode(code: string) {
    ...
    if (error || !data) return { error: 'Encomienda no encontrada' }
