  ...
  useEffect(() => {
    ...
    const {
      ...
      if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED") {
        console.log(
          "--- ADMIN DEBUG: Auth session refreshed, reloading list...",
        );
        loadConversations();
      }
    });
    ...
    const msgChannel = supabase
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
          console.log(
            "--- ADMIN DEBUG: Message subscription active for",
            selectedConvId,
          );
        }
      });
              ...
              <button
                ...
                className={`w-full p-4 flex items-start gap-3 border-b border-slate-100 hover:bg-white transition-colors text-left group ${
                  selectedConvId === chat.id
                    ? "bg-white border-l-4 border-l-chimipink shadow-sm"
                    : "border-l-4 border-l-transparent"
                }`}
              >
                <div
                  className={`p-2.5 rounded-full ${selectedConvId === chat.id ? "bg-chimipink text-white" : "bg-slate-200 text-slate-500 group-hover:bg-pink-100 group-hover:text-chimipink"}`}
                >
                  <User size={18} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-baseline mb-1">
                    <h4
                      className={`font-bold text-sm truncate ${selectedConvId === chat.id ? "text-slate-900" : "text-slate-700"}`}
                    >
          ...
          <>
                ...
                <div>
                  <h3 className="font-bold text-sm md:text-base text-slate-800 leading-tight">
                    <Link
                      ...
                      className="hover:text-chimipink hover:underline transition-colors"
                    >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col-reverse gap-4 md:gap-6">
              ...
              {[...messages].reverse().map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[85%] md:max-w-[70%] p-3 md:p-4 rounded-2xl text-sm shadow-sm ${
                      msg.is_admin
            ...
            <div className="p-4 bg-white border-t border-slate-100">
              <form
                onSubmit={handleSend}
                className="flex gap-3 items-center bg-slate-50 p-2 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-chimipink/20 focus-within:border-chimipink transition-all"
              >
                <input
                  type="text"
                  value={newMessage}
